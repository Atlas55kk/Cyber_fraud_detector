...
\begin{enumerate}
    \item \textbf{Thread Isolation:} When an investigator dispatches a trace request via \texttt{POST /api/trace/stream}, the server instantiates an isolated asynchronous queue:
    \begin{equation}
...
\begin{itemize}
    \item \textbf{\texttt{init\_stream}}: Transmits metadata, initial stolen capital, root address identity, and total anticipated search bounds.
    \item \textbf{\texttt{node\_discovered}}: Emits an individual address node record:
    \begin{equation}
        \{\text{``type''}: \text{``node''}, \text{``data''}: \{\text{``id''}: u, \text{``label''}: \text{Label}(u), \text{``risk''}: \text{Risk}(u), \text{``taint''}: \tau(u)\}\}
    \end{equation}
    \item \textbf{\texttt{wire\_discovered}}: Emits a directed transaction wire:
    \begin{equation}
        \{\text{``type''}: \text{``wire''}, \text{``data''}: \{\text{``source''}: u, \text{``target''}: v, \text{``amount''}: w_e, \text{``token''}: \kappa_e, \text{``tx\_hash''}: h_e\}\}
    \end{equation}
    \item \textbf{\texttt{heuristic\_alert}}: Dispatches high-priority forensic warnings (e.g., ``Peel Chain Detected'', ``Mixer Interaction Flagged'').
    \item \textbf{\texttt{stats\_update}}: Periodically pushes aggregated metrics (nodes explored, wires mapped, total tainted volume tracked, exploration percentage).
    \item \textbf{\texttt{stream\_complete}} / \textbf{\texttt{stream\_error}}: Finalizes the transaction session and flushes state buffers.
\end{itemize}
